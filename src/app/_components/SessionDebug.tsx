      ...
      {visible && (
        <div className="mt-2 p-4 bg-white border rounded shadow w-96 max-h-96 overflow-auto">
          ...
          <div className="mb-4">
            ...
            <div><strong>Current Holograph ID:</strong> {session?.user?.currentHolographId || 'Not set'}</div>
          </div>
          ...
          <div>
            {serverSession ? (
              <>
                <div><strong>User ID:</strong> {serverSession.user?.id || 'Not set'}</div>
                <div><strong>Email:</strong> {serverSession.user?.email || 'Not set'}</div>
                <div><strong>Current Holograph ID:</strong> {serverSession.user?.currentHolographId || 'Not set'}</div>
              </>
            ) : (
