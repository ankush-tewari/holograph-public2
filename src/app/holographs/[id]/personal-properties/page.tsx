// /src/app/holographs/[id]/personal-properties/page.tsx
...
import PersonalPropertyModal  from "@/app/_components/personal-properties/PersonalPropertyModal"
...
export default function PersonalPropertiesPage() {
  const { id: holographId } = useParams();
  const router = useRouter();
  const { data: session, status } = useSession();
  const { userId, isAuthenticated, isLoading: isHolographLoading } = useHolograph();

  const { isAuthorized, accessDenied, holographTitle, sectionName, isLoading: isAccessLoading } = useSectionAccess("personal-properties");

  const [accounts, setAccounts] = useState<PersonalProperty[]>([]);
  const [signedUrls, setSignedUrls] = useState<{ [key: string]: string }>({});
  const [selectedAccount, setSelectedAccount] = useState<PersonalProperty | null>(null);
  ...
  useEffect(() => {
    async function fetchAccounts() {
      try {
        const response = await axios.get(`/api/personal-properties?holographId=${holographId}`, { withCredentials: true });
  ...
  const openModal = (account: PersonalProperty | null) => {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this Personal Property?")) return;
    try {
      await axios.delete(`/api/personal-properties/${id}`);
    ...
    <div className="flex gap-6 p-8 max-w-6xl mx-auto">
      ...
      <div className="w-1/3 bg-white shadow-lg p-6 rounded-lg">
        <div className="flex flex-col gap-4">
          {isPrincipal && <button className="btn-primary" onClick={() => openModal(null)}>+ Add Personal Property</button>}
        ...
        <div className="mt-6 text-gray-700 text-sm space-y-2">
          {isPrincipal ? (
            <>
              <p>Use this section to list personal properties, such as jewelry, vehicles, and instructions for what to do with each personal property.</p>
              <p>You may also upload a document for each personal property with additional information. </p>
            </>
          ) : (
            <p className="italic">
              You can view Personal Properties shared with you.
            </p>
          )}
        </div>
      </div>
      ...
      <div className="w-2/3 bg-white shadow-lg p-6 rounded-lg">
        <h2 className="text-xl font-semibold text-gray-800">Personal Properties</h2>
        {isLoading ? (
          <p>Loading...</p>
        ) : accounts.length === 0 ? (
          <p className="text-gray-500">No personal properties added yet.</p>
        ) : (
          <table className="w-full mt-4 border-collapse border border-gray-300">
            <thead>
              <tr className="bg-gray-100 text-left">
                <th className="p-3 border border-gray-300">Personal Property Name</th>
      ...
      {isModalOpen && userId && (
        <PersonalPropertyModal
          userId={userId}
          personalProperty={selectedAccount}
          holographId={holographId as string}
          onClose={closeModal}
          onSuccess={refresh}
        />
      ...
      {selectedNote !== null && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={`bg-white p-6 rounded-lg shadow-lg ${isExpanded ? 'max-w-5xl w-full h-[90vh]' : 'max-w-md w-full max-h-[80vh]'} overflow-y-auto relative transition-all duration-300`}>
            <h2 className="text-lg font-semibold mb-4">Personal Property Notes</h2>
